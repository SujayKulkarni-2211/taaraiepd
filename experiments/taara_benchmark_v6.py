#!/usr/bin/env python3
"""
TAARA Benchmark v6 — Command-Only Attack Class (Behavioral Scope)
=============================================================================

Scope clarification over v5:
  TAARA is a BEHAVIORAL continuous authentication system.
  A session with zero commands has no behavior to analyze.

  v5 included 2871/3000 zero-command Cowrie sessions (port scanners / credential
  stuffers that authenticated but immediately disconnected without running anything).
  These trivially confuse every behavioral method because all behavioral features
  are 0 — indistinguishable from an idle normal session.

  v6 restricts the attack class to Cowrie sessions with ≥1 command executed.
  This is the correct scope: TAARA detects behavioral drift during ACTIVE sessions.
  Zero-command sessions are a different threat class handled by IDS/firewall, not UEBA.

  This is NOT cherry-picking. We state explicitly:
  "TAARA analyzes sessions with observed command activity. Sessions with no commands
  have no behavioral signal and cannot be analyzed by any UEBA method."

v6 also activates per-identity weight fitting (α,β,γ via logistic regression on
a 20% held-out validation split per identity) — v5 left these hardcoded at the
global prior (0.263, 0.285, 0.451). Weight fitting runs BEFORE the test evaluation.
...
Three features DISCOVERED by transformer attention (not hand-engineered):
  hardware_enum_count  — uname+free+top+lscpu+w cluster (1087-1092/1100 attack sessions)
  persistence_attempt  — crontab specifically (1087/1100 attack sessions)
  network_device_shell — version/shell/enable = router/IoT exploitation (0/1100 normal)

Datasets:
  Normal: elastic_auth.log + SSH.log legit (1151 sessions, 20 identities)
  Attack: Cowrie Zenodo 3687527 — sessions with ≥1 command (behavioral scope)

Comparators:
  1. TAARA_v6 — per-identity quantum SWAP + coherence fusion + fitted weights
  2. IF_global — IsolationForest global (what Splunk/Sentinel approximate)
  3. IF_per_identity — per-user IsolationForest (fairest classical comparison)
  4. LOF_per_identity
  5. PerUser_ZScore — simple SIEM threshold
  6. LSTM_AE — academic deep learning baseline

Usage:
    source venv/bin/activate && python experiments/taara_benchmark_v6.py

Output:
    experiments/results/benchmark_v6_results.json
    experiments/results/benchmark_v6_report.txt
"""
...
def evaluate(scores: np.ndarray, labels: np.ndarray, name: str,
    ...
    """Threshold = 90th percentile of normal training scores."""
...
def main():
    ...
    # ── 6. TAARA v6 inference (per-identity) ──────────────────────────────────
    print("\n[6] TAARA v6 inference (per-identity quantum SWAP + fitted weights)...")
    ...
    print("  "+"-"*70)
    results = {}
    results["TAARA_v6"]           = evaluate(taara_scores,    test_labels, "TAARA_v6")
    ...
    output = {
        ...
        "benchmark":  "TAARA v6 — command-only attack class (behavioral scope)",
        "version":    "6.0",
        "datasets": {
            "normal": f"elastic_auth.log ({len(elastic)} sessions) + SSH.log legit ({len(ssh_legit)} sessions)",
            "attack": f"Cowrie Zenodo 3687527 — {len(cowrie)} sessions with ≥1 command (behavioral scope; {len(cowrie_all)-len(cowrie)} zero-command excluded)",
        },
        "features": {
            "count": 19,
        ...
        "hard_case_analysis": {
            ...
            "scope_note": "command-only sessions only; zero-command sessions excluded from benchmark",
        },
        "runtime_seconds": round(elapsed,1),
    }

    out_j = RESULTS_DIR/"benchmark_v6_results.json"
    with open(out_j,"w") as f: json.dump(output,f,indent=2)
    print(f"\n  Results → {out_j}")

    # Text report
    lines = [
        "TAARA Benchmark v6 Report — Command-Only Attack Class (Behavioral Scope)",
        "="*60,
        f"Normal: {len(normal)} sessions ({len(elastic)} elastic + {len(ssh_legit)} ssh_legit)",
        f"Attack: {len(cowrie)} Cowrie sessions with ≥1 command (Zenodo 3687527)",
        f"  Excluded: {len(cowrie_all)-len(cowrie)} zero-command sessions (no behavioral signal)",
    ...
    out_t = RESULTS_DIR/"benchmark_v6_report.txt"
